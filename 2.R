...
heat_countries <- c(
  "Burkina Faso", "Burundi", "Cameroon", "Chad", 
  "Côte d'Ivoire", "Mauritania", "Nigeria", "Rwanda", 
  "Senegal", "South Africa", "Togo", "Zimbabwe"
)
...
ggplot() +
  ...
  scale_fill_manual(
    values = c("TRUE" = alpha("#FFE0B2", 0.7),  # Light yellow for HEAT countries
               "FALSE" = "white"),               # White for non-HEAT countries
    name = NULL,
    labels = c("HEAT Contributing Countries", ""),
    breaks = c("TRUE", "FALSE"),
    guide = guide_legend(
      override.aes = list(color = NA),  # Remove border in legend
      order = 2  # Place below the project participation legend
    )
  ) +
