...
# Create partners dataset with updated African institutions
partners_data <- tribble(
  ...
  "Africa Climate Development Institute", "Cape Town", "South Africa", -33.9249, 18.4241, "Multiple Projects", c("CHAMNHA", "HE²AT"),
  "Quantium Health", "Johannesburg", "South Africa", -26.2041, 28.0473, "HE²AT", c("HE²AT"),
  "Section27", "Johannesburg", "South Africa", -26.2041, 28.0473, "Multiple Projects", c("CHAMNHA", "HE²AT"),
...
partners_sf <- partners_sf %>%
  mutate(short_name = case_when(
    ...
    str_detect(institution, "Africa Climate Development") ~ "ACDI",
    str_detect(institution, "Quantium") ~ "Quantium",
    str_detect(institution, "Section27") ~ "Section27",
    TRUE ~ institution
  ))
...
ggplot() +
  ...
  labs(
    ...
    subtitle = "Extended Research Collaboration Map",
    caption = "Data: Research Collaboration Network | Map: 2025"
  )

# Save outputs
ggsave("african_partners_map_extended_2025.pdf", 
       width = 14, 
       height = 14,
       dpi = 350)

ggsave("african_partners_map_extended_2025.png", 
       width = 14, 
       height = 14,
       dpi = 350)