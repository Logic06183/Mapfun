...
# Load Google Fonts - using a more readable font
...
# Define all HE²AT study countries (expanded list)
heat_countries <- c(
  # Original countries
  "South Africa", "Zimbabwe", "Nigeria", "Ghana", 
  "Cameroon", "Benin", "Burkina Faso", "Côte d'Ivoire",
  # Additional countries from Image 2
...
# Create partners dataset - unchanged
...
# Add project count as explicit column
partners_sf <- partners_sf %>%
  mutate(project_count = sapply(projects, length))

# Split countries into groups to avoid overcrowding
# Prioritize countries with partner institutions
countries_with_partners <- unique(partners_data$country)

# Create better color scheme for different project types
project_colors <- c(
  "CHAMNHA" = "#1E88E5",       # Stronger blue
  "HE²AT" = "#00897B",         # Teal green
  "Multiple Projects" = "#D81B60"  # Magenta
)

# Prepare highlighted countries with category label
highlighted_countries <- africa %>% 
  filter(name %in% heat_countries) %>%
  mutate(category = "HE²AT Contributing Countries")

# Create the improved map
ggplot() +
  # Base layer - world boundaries for context
  geom_sf(data = africa_borders,
          fill = "#F5F5F5",
          color = "gray90",
          size = 0.1) +
  
  # African continent with subtle texture
  geom_sf(data = africa, 
          fill = "#FAFAFA", 
          color = "gray80", 
          size = 0.2) +
  
  # Add subtle graticules (latitude/longitude grid)
  geom_sf(data = st_graticule(lat = seq(-30, 30, 10), lon = seq(-20, 40, 10), crs = 4326),
          color = alpha("gray70", 0.3),
          size = 0.1) +
  
  # Add all HE²AT study countries with improved styling - with category mapping
  geom_sf(data = highlighted_countries,
          aes(fill = category),  # Use the category field for legend
          color = "#FF9800",    
          size = 0.5) +
  
  # Add partner points with improved styling
  ...
  # Add halos behind points for emphasis
  geom_sf(data = partners_sf,
  ...
  # Add country name labels for partner countries only
  geom_sf_text(
    data = africa %>% 
  ...
  # Add improved city labels with better positioning
  geom_text_repel(
    data = partners_sf,
  ...
  # Customize colors with improved palette
  scale_color_manual(
    values = project_colors,
    name = "Project Type"
  ) +
  
  # Set fill scale for countries - proper legend
  scale_fill_manual(
    values = alpha("#FFB74D", 0.4),
    name = NULL
  ) +
  
  # Better size scaling with clearer breaks
  ...
  # Control legend order and position
  guides(
  ...
  # Set map extent - further adjusted to show more of Africa
  coord_sf(
    xlim = c(-20, 50),
    ylim = c(-40, 40),
    expand = FALSE
  ...
  # Add titles with improved wording and updated year
  labs(
    title = "African Research Partners Network",
    subtitle = "Wits Planetary Health Research Collaboration Map",
    caption = "Data: WitsPHR | Map: Craig Parker, 2025"
  )

# Save the map with improved resolution
ggsave("final_african_partners_map_2025.pdf", 
       width = 14, 
       height = 14,
       dpi = 350)

# Also save as high-quality PNG
ggsave("final_african_partners_map_2025.png", 
       width = 14, 
       height = 14,
       dpi = 350)